<!DOCTYPE html>
<?php 
	require_once('config.php');
	include 'sql_db/db_sql.php';

	if($GLOBALS['eveonline']['db']['id'] == '< your client id >'){ die('missing client_id: change config.php - line 24');}
	if($GLOBALS['eveonline']['db']['base64'] == '< base64 encoded secret >'){ die('missing base64 secret: change config.php - line 25');}
	$client_id = $GLOBALS['eveonline']['db']['id'];
	
	$sql = "SELECT * FROM eve_oauth WHERE id != ''";
	if(!($result = $db->sql_query($sql))){	
		$url = "https://login.eveonline.com/oauth/authorize/?response_type=code&redirect_uri=http://localhost/callback/&client_id=".$client_id."&scope=esi-wallet.read_character_wallet.v1";
		print '<a href="'.$url.'"><img src="image/ui/eve-sso-login-white-large.png"></a>';
	}else{
		while(($row = $db->sql_fetchrow($result))){ 
			$char[] = $row;
		}
	}
?>
<html><head>
<title>ESI-oAuth2.0 || Eve Online SSO</title>
<style>
body {
	background-color:#fff;
	font: 12px tahoma,sans-serif normal;
	color: #000;
	text-decoration: none;
}
a.char_type:link, a.char_type:visited, a.char_type:active, a.char_type:hover {
	text-decoration:none;
	color:#000;
}
a.char_type:hover{
	color:#666;
}

.icon {
	margin-top:3px;
	border:2px solid #888;
}
</style>
</head>
<body>

<?php
	
	if($char != ""){
		foreach($char as $x){
			$name = $x['name'];
			$id = $x['id'];
			$scope = $x['scope'];
	
			print '<span style="display:block;height:64px;width:640px;padding:5px;background:#ccc;margin-bottom:5px">
				<span style="float:left;margin-right:5px">
				<img src="https://imageserver.eveonline.com/Character/'.$id.'_256.jpg" width="64" align="top">
				</span>'.$name.' --[char_id: '.$id.']<br>'.$scope.'<br>
				<a href="transactions.php?id='.$id.'" class="char_type"><img src="image/ui/wallet_icon" class="icon"></a>
				<a href="journal.php?id='.$id.'" class="char_type"><img src="image/ui/journal_icon" class="icon"></a>
				</span>';
		}
		print '<a href="'.$url.'"><img src="image/ui/eve-sso-login-white-large.png"></a>';
	}
	
?>
</body>
</html>